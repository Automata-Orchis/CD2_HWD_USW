...
<!-- frontend에서 Model, Device, Work, Analysis 버튼의 배치를 조정해야 한다. 

현재 Model 옆 모델 로드 버튼과 Device 사이의 간격이 너무 넓다. 동시에 Work로서 선택된 신청서의 이름의 길이에 따라 버튼의 위치가 변하는 것을 확인했다. 

frontend에서 선택하는 버튼의 위치는 반드시 고정해라. 만일 Work로 선택된 신청서의 이름이 길어 보이지 않는다면 마우스 커서를 위호 올렸을 때 그 전체 이름이 뜨도록 수정해라. 

---

분석이 끝난 신청서는 표기를 했으면 한다. 분석이 안 된 것인지 혹은 모델 추론 오류인지 구분할 수가 없어, 어떤 신청서의 분석이 끝난 건지 확인이 안 된다. 이래선 모든 작업이 끝날 때까지 정보 수정 작업을 시작할 수 없다.  -->
